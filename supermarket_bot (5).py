...
def parse_report(text: str) -> dict:
    ...
    # Monthly total: look for "MARCH SALES", "MONTHLY SALES", or "★/⭐ ... SALES"
    m = re.search(r'(?:⭐️?|★)?\s*\w+ SALES\s*:?\s*[₱]?\s*([\d,]+\.?\d*)', text, re.IGNORECASE)
    d['monthly_total'] = float(m.group(1).replace(',', '')) if m else 0.0

    # TOTAL CASH = cash currently in drawer (separate from monthly total)
    tc = re.search(r'TOTAL CASH\s*:?\s*[₱]?\s*([\d,]+)', text, re.IGNORECASE)
