...
def parse_report(text: str) -> dict:
    ...
    # Graveyard: match "Graveyard" or "Grave yard shift"
    ...
    d['total_cash_drawer'] = float(tc.group(1).replace(',', '')) if tc else 0.0
...
def save_record(data: dict, raw_text: str, chat_id: int):
    ...
    c.execute('''
        ...
        (date, store, submitted_by, cash_sale, card_sale, qr_ph, maya, grab,
         ...
         cashbox, for_deposit, foodpanda, cash_drawer, raw_text, chat_id)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    ''', (
        ...
        data['cashbox'], data['for_deposit'],
        data.get('foodpanda', 0), data.get('total_cash_drawer', 0),
        raw_text, chat_id
    ))
    conn.commit()
    conn.close()
...
💵 レジ現金: ₱{data.get('total_cash_drawer', 0):,.0f}
...
📆 営業日数: {days_count}日"""
    ...
    # Trend + Shift chart
    buf_trend = make_trend_chart(records, "Monthly Sales Trend")
    await update.message.reply_photo(photo=buf_trend, caption="📈 Sales Trend")

    buf_shift = make_shift_chart(records)
    await update.message.reply_photo(photo=buf_shift, caption="📊 Sales by Shift")
...
async def cmd_compare(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == 'shift':
        ...
        await update.message.reply_photo(photo=buf, caption="📊 Shift Comparison (Last 30 days)")
    else:
        buf = make_payment_chart(records)
        await update.message.reply_photo(photo=buf, caption="💳 Payment Method Comparison (Last 30 days)")
...
最低売上: ₱{min(r['total'] for r in records):,.0f}（{min(records, key=lambda r: r['total'])['date']}）"""
    ...
    await update.message.reply_text(text)
    buf = make_trend_chart(records, "30-Day Trend")
...
async def is_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> bool:
    """Check if the sender is a group admin (or owner), or in private chat."""
...
async def cmd_delete(update: Update, ctx: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if date_match:
        ...
        if not row:
            ...
            return
        store = row[0]
        if delete_record_db(date_str, store, chat_id):
            await update.message.reply_text(f"🗑️ {store} の {date_str} のレポートを削除しました。")
        else:
            await update.message.reply_text(f"⚠️ 削除に失敗しました。")
    else:
        # 最新レポートを削除
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # 2) ボットへの話しかけ（@mention or DM）のみ反応
    ...
    if intent == 'weekly':
        ...
        await cmd_monthly(update, ctx)
    elif intent == 'compare_shift':
        ctx.args = ['shift']
        await cmd_compare(update, ctx)
    elif intent == 'compare_payment':
        ctx.args = ['payment']
        await cmd_compare(update, ctx)
    elif intent == 'trend':
        await cmd_trend(update, ctx)
    elif intent == 'export':
        await cmd_export(update, ctx)
    elif intent == 'english':
        await cmd_english(update, ctx)
    elif intent == 'help':
        await cmd_help(update, ctx)
    elif intent == 'delete':
        await cmd_delete(update, ctx, text)
    else:
        help_text = """🤖 話しかけてくれてありがとう！こんなことができるよ：
...
📁 「CSVダウンロード」— データ出力
🇬🇧 「英語にして」— 英語レポート生成
🗑️ 「最新レポートを削除」— 最新データ削除（管理者のみ）
🗑️ 「2026-03-08のレポートを削除」— 日付指定削除（管理者のみ）"""
...
def main():
    ...
    # Single message handler for everything (natural language + report detection)
