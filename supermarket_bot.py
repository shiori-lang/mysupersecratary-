...
async def cmd_weekly(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for r in records:
        try:
            ...
            day = dow_map[str(d.weekday())]
            dow_sales[day] = dow_sales.get(day, 0) + r['total']
        except:
            pass
    dow_line = ' | '.join(f"{k}:₱{v:,.0f}" for k, v in dow_sales.items())
    ...
    wow_pct = (total_sum - prev_total) / prev_total * 100 if prev_total > 0 else 0
    wow_line = f"📊 前週比: {wow_pct:+.1f}% (₱{total_sum-prev_total:+,.0f})" if prev_total > 0 else "📊 前週データなし"

    # Payment trend
    prev_cash_pct = prev_cash / prev_total * 100 if prev_total > 0 else 0
    curr_cash_pct = cash_sum / total_sum * 100 if total_sum > 0 else 0
    cash_trend = "↑" if curr_cash_pct > prev_cash_pct else "↓"
    prev_grab_pct = prev_grab / prev_total * 100 if prev_total > 0 else 0
    curr_grab_pct = grab_sum / total_sum * 100 if total_sum > 0 else 0
    grab_trend = "↑" if curr_grab_pct > prev_grab_pct else "↓"

    def pct(v): return v / total_sum * 100 if total_sum > 0 else 0

    text = f"""📅 週次サマリー（直近7日）
━━━━━━━━━━━━━━━━━━━
💰 総売上:  ₱{total_sum:,.0f}
📊 日平均:  ₱{total_sum/len(records):,.0f}
👥 総取引: {tx_sum}件（日平均 {tx_sum//len(records)}件）
{wow_line}

【決済別】
💵 現金:  ₱{cash_sum:,.0f} ({pct(cash_sum):.1f}%) {cash_trend}
💳 カード: ₱{card_sum:,.0f} ({pct(card_sum):.1f}%)
📱 QR/MAYA: ₱{qr_sum+maya_sum:,.0f} ({pct(qr_sum+maya_sum):.1f}%)
🚗 Grab:  ₱{grab_sum:,.0f} ({pct(grab_sum):.1f}%) {grab_trend}
...
【損失】
⚠️ 廃棄合計: ₱{wastage_s:,.0f} ({wastage_s/total_sum*100 if total_sum else 0:.1f}%)
🏷️ 値引合計: ₱{discount_s:,.0f} ({discount_s/total_sum*100 if total_sum else 0:.1f}%)

🏆 最高売上: {best['date']} ₱{best['total']:,.0f}
📉 最低売上: {worst['date']} ₱{worst['total']:,.0f}"""
    ...
    try:
        client = anthropic.Anthropic(api_key=ANTHROPIC_KEY)
        prompt = f"""週次売上データを分析し、オーナーへの洞察に富んだコメントを日本語で3〜4点、箇条書きで生成してください。
総売上: ₱{total_sum:,.0f} | 前週比: {wow_pct:+.1f}% | 日平均: ₱{total_sum/len(records):,.0f}
現金比率: {pct(cash_sum):.1f}% | Grab比率: {pct(grab_sum):.1f}%
最高売上日: {best['date']} ₱{best['total']:,.0f} | 最低売上日: {worst['date']} ₱{worst['total']:,.0f}
廃棄率: {wastage_s/total_sum*100 if total_sum else 0:.1f}% | 値引率: {discount_s/total_sum*100 if total_sum else 0:.1f}%
コメントのみ返答してください。"""
        resp = client.messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=400,
        ...
        await update.message.reply_text(f"💡 週次AIコメント\n{resp.content[0].text.strip()}")
    except Exception as e:
        logger.error(f"Weekly AI comment error: {e}")
